...
export default function RoomForm({ onRoomSaved, onCancel, room }: RoomFormProps) {
    ...
    return (
        <form onSubmit={handleSubmit} className="room-form">
            ...
            <textarea placeholder="Полное описание" value={description} onChange={(e) => setDescription(e.target.value)} />
            ...
            <div className="features-section">
                ...
                {features.map((feature, index) => (
                    <div key={index} className="feature-item">
                        <input type="text" placeholder="Название" value={feature.label} 
                               onChange={(e) => updateFeature(index, 'label', e.target.value)} />
                        <FileUpload slug={slug} type="icon" accept="image/svg+xml,image/png"
                                    onUploaded={(url) => updateFeature(index, 'icon', url)} />
                        {feature.icon && <Image src={feature.icon} alt="icon preview" width={30} height={30} />}
                        <button type="button" onClick={() => removeFeature(index)}>Удалить</button>
                    </div>
                ))}
            ...
            <div className="images-section">
                ...
                {images.map((image, index) => (
                    <div key={index} className="image-item">
                        <FileUpload slug={slug} type="image" accept="image/jpeg,image/webp"
