...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadDir = path.join(process.cwd(), 'public', 'upload', 'rooms', slug, subFolder);
    ...
    return NextResponse.json({ error: 'Внутренняя ошибка при загрузке файла' }, { status: 500 });
  }
}
